...
func handle(evt: Dictionary) -> void:
	...
	var id := str(evt.get("agent", "")).strip_edges()
	# A blank/missing agent id has no subject to animate — never conjure the
	# literal "agent" ghost (it hash-rolls to "Agent (Analyst)" and gets stuck on
	# the wallpaper). Events carrying only agents[] are handled above.
	if id == "":
		return
